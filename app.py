...
def analyze_food():
    ...
    try:
        ...
        กฎเหล็กการใช้หน่วยวัดที่ต้องปฏิบัติตามอย่างเคร่งครัดตามประเภทอาหาร:
        1. ห้ามใช้คำว่า "หน่วยบริโภค" หรือ "ส่วน" เด็ดขาด
        2. หมวดผลไม้ทั้งหมด (เช่น กล้วย, ส้ม, แอปเปิ้ล, มะม่วง, ลิ้นจี่, มังคุด, ฝรั่ง) ต้องระบุหน่วยเป็น **"ผล"** หรือ **"ลูก"** (เช่น 1 ผล, 2 ลูก, 1/2 ลูก) เสมอ
        3. หมวดข้าว แป้ง และเส้น (เช่น ข้าวสวย, ข้าวกล้อง, ข้าวเหนียว, เส้นก๋วยเตี๋ยว, ขนมจีน) ต้องระบุหน่วยเป็น **"ทัพพี"** เสมอ
        4. หมวดอาหารคาว เนื้อสัตว์ เมนูปิ้งย่าง (เช่น หมูย่าง, ไก่ย่าง) ให้ระบุเป็น **"กรัม"**, **"ขีด"** หรือ **"ชิ้น"**
        5. หมวดเครื่องปรุงรส ให้ระบุเป็น **"ช้อนโต๊ะ"** หรือ **"ช้อนชา"**

        คุณต้องตอบกลับมาในรูปแบบ JSON ที่มีคีย์ (Keys) ตรงตามนี้เท่านั้น ห้ามใส่เครื่องหมายอื่นครอบโค้ด JSON:
        {{
            "normal_amount": "ระบุปริมาณที่แนะนำสำหรับคนปกติโดยใช้หน่วยที่ถูกต้องตามประเภทอาหารข้างต้น (เช่น ทัพพี, ผล, ลูก, ชิ้น, ช้อนโต๊ะ)",
        ...
        q_lower = query.lower()
        if any(f in q_lower for f in ["กล้วย", "ส้ม", "แอปเปิ้ล", "มะม่วง", "ลิ้นจี่", "เงาะ", "มังคุด", "ฝรั่ง"]):
            fallback_normal = "1 - 2 ผล (หรือ 1 - 2 ลูก)"
